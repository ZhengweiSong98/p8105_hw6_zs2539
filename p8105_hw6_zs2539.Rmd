...
```
* Based on t-test, the variables with p-value less than 0.01 are to be selected as potential predictors. To simple, the continuous are to be selected.
...
```
* `bhead`, `blength`, `fincome`,`gaweeks`, `smoken` showed a relatively significant linear relationship with birth weight.
...
cv_df %>% 
  ...
  theme(plot.title = element_text(hjust = 0.5, face = "bold"))
```
* Based on cross-validation, my model has the relatively lowest root mean square error and therefore my model works better.
